...
\end{lemma}
\begin{proof}
Recall that $D^*$ is dominating set. And let we call ~$ \left|V\setminus \bigcup_{v\in {\cal D}}\,N[v]\right|$ is $(A)$. So the union of $N[v]$ over all $v$ $\in$ $D^*$ equals $V$, which is a superset of $(A)$. And then we can say some $D^*$ can cover~$\left(V\setminus \bigcup_{v\in {\cal D^*}}\,N[v]\right)$, cover at least a $\frac{1}{|D^*|}$  proportion of .
