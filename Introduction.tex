...
\end{lemma}
\begin{proof}
$D^*$ is dominating set. So the union of $N[v]$ over all $v$ $\in$ $D^*$ equal $V$. Therefore $D^*$'s each vertex have cover~$\left(V\setminus \bigcup_{v\in {\cal D}}\,N[v]\right)$. And then we can say $D^*$ has some vertex can cover~$\left(V\setminus \bigcup_{v\in {\cal D}}\,N[v]\right)$, at least $\frac{1}{|D^*|}$ proportion.
