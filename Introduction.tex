...
\end{lemma}
\begin{proof}
Recall that $D^*$ is a dominating set. And let us call ~(\ref{lemma2-equation}). So the union of $N[v]$ over all $v$ $\in$ $D^*$ equals $V$, which is a superset of ~(\ref{lemma2-equation}). And then we can say some $u\in D^*$ can cover~$\left(V\setminus \bigcup_{v\in {\cal D}}\,N[v]\right)$ cover at least a $\frac{1}{|D^*|}$  proportion of ~(\ref{lemma2-equation}).
\end{proof}
\clearpage
