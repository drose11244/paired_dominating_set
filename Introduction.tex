...
\end{lemma}
\begin{proof}
Recall that $D^*$ is dominating set. So the union of $N[v]$ over all $v$ $\in$ $D^*$ equals $V$, which is a superset of $N[v]$ over all $v$ in $D^*$. And then we can say some each $D^*$ can cover~$\left(V\setminus \bigcup_{v\in {\cal D^*}}\,N[v]\right)$, at least proportion of $\frac{1}{|D^*|}$ .
